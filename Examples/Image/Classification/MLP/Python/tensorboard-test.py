...
data_path = os.path.join(os.path.dirname("E:\\repos\\cntk\\Examples\\Image\\DataSets\\MNIST"), "MNIST")
...
def create_reader(path, is_training, input_dim, label_dim):
    ...
    )), randomize=is_training, max_sweeps=3 if is_training else 1)


# Creates and trains a feedforward classification model for MNIST images
#def convnet_mnist(debug_output=False, epoch_size=60000, minibatch_size=64, max_epochs=40):
def convnet_mnist(debug_output=False, epoch_size=60000, minibatch_size=64, max_epochs=3):
    image_height = 28
    image_width = 28
    num_channels = 1
    ...
    dummy = C.reshape(input_var, (num_channels, image_height, image_width))
    label_var = C.ops.input_variable(num_output_classes, np.float32)

    # Instantiate the feedforward classification model
    scaled_input = C.ops.element_times(C.ops.constant(0.00390625), dummy)
    ...
    tensorboard_writer = C.logging.TensorBoardProgressWriter(freq=10, log_dir='log', model = z)
    #progress_printer = C.logging.ProgressPrinter(tag='Training', num_epochs=max_epochs)
    trainer = C.Trainer(z, (ce, pe), learner, tensorboard_writer)
    ...
    for epoch in range(max_epochs):  # loop over epochs
        ...
        while sample_count < epoch_size:  # loop over minibatches in the epoch
            ...
            output = trainer.train_minibatch(data, outputs=[input_var])  # update model with it
            flag = output[0]
            dict = output[1]
            sample_count += data[label_var].num_samples  # count samples processed so far
#            image_value = dummy.eval(data, as_numpy=False)
            image_value = dict[input_var]
            tensorboard_writer.write_image('test', dict, sample_count)
